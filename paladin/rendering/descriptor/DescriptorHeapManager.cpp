...
bool DescriptorHeapManager::CreateDescriptorHeap(ID3D12Device* device, std::uint32_t hash, D3D12_DESCRIPTOR_HEAP_DESC description)
{
 ...
 ViewHandle DescriptorHeapManager::CreateShaderResourceView(ID3D12Device* device, std::uint32_t hash,ID3D12Resource* resource, D3D12_SHADER_RESOURCE_VIEW_DESC* desc )
    {
        if (std::optional<DescriptorHeap> heap_result = Get(hash); heap_result.has_value())
        {
            auto& heap = heap_result.value();
...
std::optional<DescriptorHeap> DescriptorHeapManager::Get(const std::uint32_t hash)
{
    if (hash_to_heap.contains(hash))
    {
...
D3D12_CPU_DESCRIPTOR_HANDLE DescriptorHeapManager::GetCPUHandle(const ViewHandle& handle)
{
    ...
    if (const std::optional<DescriptorHeap> heap_result = Get(handle.heap_identifier); heap_result.has_value())
    {
        auto start_handle = heap_result.value().StartCPU();
        start_handle.ptr += handle.index*heap_result.value().DescriptorIncrementSize();
        return start_handle;
    }
    return {};
}
...
D3D12_GPU_DESCRIPTOR_HANDLE DescriptorHeapManager::GetGPUHandle(const ViewHandle& handle)
{
    ...
    if (const std::optional<DescriptorHeap> heap_result = Get(handle.heap_identifier); heap_result.has_value())
    {
        auto start_handle = heap_result.value().StartGPU();
        start_handle.ptr += handle.index*heap_result.value().DescriptorIncrementSize();
        return start_handle;
    }
    return {};
